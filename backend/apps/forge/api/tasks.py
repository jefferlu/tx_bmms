...
def process_translation(urn, token, file_name, object_data, send_progress):
    ...
    with transaction.atomic():
        ...
        obj_df['child'] = obj_df['child'].apply(lambda x: [int(c) for c in x.split(',')] if pd.notnull(x) else [])
        obj_df['parent'] = obj_df['parent'].apply(lambda x: int(x) if pd.notnull(x) else None)
        obj_df['is_leaf'] = obj_df['is_leaf'].astype(bool)
