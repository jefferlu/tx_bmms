...
export class ApsViewerComponent implements OnInit, AfterViewInit, OnChanges, OnDestroy {
    ...
    private handleModelLoading(data: any[]): void {
        ...
        viewer.addEventListener(Autodesk.Viewing.GEOMETRY_LOADED_EVENT, () => {
            ...
            // 載入 AEC 擴展
            this.viewer.loadExtension('Autodesk.AEC.ModelData').then(() => {
                console.log('Autodesk.AEC.ModelData 載入成功');
            }).catch((err) => {
                console.error('無法載入 Autodesk.AEC.ModelData:', err);
                this._toastService.open({ message: '無法載入樓層選擇功能' });
            });
        });
    }
