...
export class ApsViewerComponent implements OnInit, AfterViewInit, OnChanges, OnDestroy {
    ...
    private unitTest() {
        ...
        const mockManifest = {
            ...
            derivatives: [
                {
                    name: 'T3-TP16-XXX-XX-XXX-M3-XX-00001.nwd',
                    status: 'success',
                    outputType: 'svf',
                    children: [
                        {
                            guid: '03148620-f884-c1e7-ac8d-ded5b5e4f2bf',
                            type: 'geometry',
                            role: '3d',
                            name: 'T3-TP16-XXX-XX-XXX-M3-XX-00001.nwd',
                            status: 'success',
                            path: 'assets/aps/svf/model4/0.svf' // 本地 SVF 文件路徑
                        },
                        {
                            guid: '3b777035-2c46-4dc7-a2fc-92934ea7a0d3',
                            type: 'resource',
                            role: 'Autodesk.AEC.ModelData',
                            status: 'success',
                            path: 'assets/aps/svf/model4/AECModelData.json' // 本地 AEC 數據文件
                        }
                    ]
                }
            ]
        ...
        Autodesk.Viewing.Initializer(options, () => {
            this.viewer.init(container, options).then(() => {
                ...
                // 模擬本地 manifest 和 SVF 載入
                this.loadLocalModel(mockManifest, svfPaths, options).then(() => {
                    console.log('所有本地模型載入成功');
                }).catch((err) => {
                    console.error('本地模型載入失敗:', err);
                    this._toastService.open({ message: '無法載入本地模型' });
                });
    ...
    private loadLocalModel(manifest: any, svfPaths: string[], options: any): Promise<void> {
        const loadPromises: Promise<any>[] = [];

        // 模擬 URN 和 manifest 結構
        const documentId = `urn:${manifest.urn}`;

        // 為 AggregatedView 準備節點
        const nodes: any[] = [];

        // 處理 mockManifest 中的 derivatives
        manifest.derivatives.forEach((derivative: any) => {
            const svfChild = derivative.children.find((child: any) => child.outputType === 'svf' || child.role === '3d');
            if (svfChild && svfChild.path) {
                loadPromises.push(
                    new Promise((resolve, reject) => {
                        // 使用 Autodesk.Viewing.Document.load 載入本地 SVF
                        Autodesk.Viewing.Document.load(
                            svfChild.path, // 本地 SVF 文件路徑
                            (doc) => {
                                ...
                                if (viewables.length > 0) {
                                    ...
                                    console.log(`本地模型載入成功: ${svfChild.path}`);
                                    resolve({ doc, urn: documentId });
                                } else {
                                    reject(new Error(`無法找到 3D 視圖: ${svfChild.path}`));
                                }
                            },
                            (errorCode, errorMsg) => {
                                console.error(`無法載入本地模型 ${svfChild.path}: ${errorMsg}`);
                                reject(new Error(`無法載入本地模型 ${svfChild.path}: ${errorMsg}`));
                            },
                            options
                        );
                    })
                ...
                new Promise((resolve, reject) => {
                    Autodesk.Viewing.Document.load(
                        ...
                        (doc) => {
                            ...
                            if (viewables.length > 0) {
                                nodes.push({
                                    urn: `urn:local:custom_${path}`,
                                    viewable: viewables[0]
                                });
                                console.log(`本地模型載入成功: ${path}`);
                                resolve({ doc, urn: `urn:local:custom_${path}` });
                            } else {
                                reject(new Error(`無法找到 3D 視圖: ${path}`));
                            }
                        },
                        (errorCode, errorMsg) => {
                            console.error(`無法載入本地模型 ${path}: ${errorMsg}`);
                            reject(new Error(`無法載入本地模型 ${path}: ${errorMsg}`));
                        },
                        options
                    );
                })
        ...
        return Promise.all(loadPromises).then(() => {
            // 將所有節點設置到 AggregatedView
            this.viewer.setNodes(nodes);
        });
    }
