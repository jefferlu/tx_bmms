...
import { GTS_MOCK_API_DEFAULT_DELAY } from '@gts/lib/mock-api/mock-api.constants';
...
{
    const defaultDelay = inject(GTS_MOCK_API_DEFAULT_DELAY);
    const gtsMockApiService = inject(GtsMockApiService);

    // Try to get the request handler
    const {
        handler,
        urlParams,
    } = gtsMockApiService.findHandler(request.method.toUpperCase(), request.url);
