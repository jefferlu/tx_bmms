...
import { GtsNavigationItem, GtsVerticalNavigationAppearance, GtsVerticalNavigationMode, GtsVerticalNavigationPosition } from '@gts/components/navigation/navigation.types';
import { GtsVerticalNavigationAsideItemComponent } from '@gts/components/navigation/vertical/components/aside/aside.component';
...
import { GtsScrollbarDirective } from '@gts/directives/scrollbar/scrollbar.directive';
...
export class GtsVerticalNavigationComponent implements OnChanges, OnInit, AfterViewInit, OnDestroy
{
    ...
    @Input() appearance: GtsVerticalNavigationAppearance = 'default';
    @Input() autoCollapse: boolean = true;
    @Input() inner: boolean = false;
    @Input() mode: GtsVerticalNavigationMode = 'side';
    @Input() name: string = this._gtsUtilsService.randomId();
    @Input() navigation: GtsNavigationItem[];
    @Input() opened: boolean = true;
    @Input() position: GtsVerticalNavigationPosition = 'left';
    @Input() transparentOverlay: boolean = false;
    @Output() readonly appearanceChanged: EventEmitter<GtsVerticalNavigationAppearance> = new EventEmitter<GtsVerticalNavigationAppearance>();
    @Output() readonly modeChanged: EventEmitter<GtsVerticalNavigationMode> = new EventEmitter<GtsVerticalNavigationMode>();
    @Output() readonly openedChanged: EventEmitter<boolean> = new EventEmitter<boolean>();
    @Output() readonly positionChanged: EventEmitter<GtsVerticalNavigationPosition> = new EventEmitter<GtsVerticalNavigationPosition>();
    @ViewChild('navigationContent') private _navigationContentEl: ElementRef;

    activeAsideItemId: string | null = null;
    onCollapsableItemCollapsed: ReplaySubject<GtsNavigationItem> = new ReplaySubject<GtsNavigationItem>(1);
    onCollapsableItemExpanded: ReplaySubject<GtsNavigationItem> = new ReplaySubject<GtsNavigationItem>(1);
    ...
    private _gtsScrollbarDirectives!: QueryList<GtsScrollbarDirective>;
    private _gtsScrollbarDirectivesSubscription: Subscription;
    ...
    @HostBinding('class') get classList(): any
    {
        /* eslint-disable @typescript-eslint/naming-convention */
        return {
            'gts-vertical-navigation-animations-enabled'             : this._animationsEnabled,
            [`gts-vertical-navigation-appearance-${this.appearance}`]: true,
            'gts-vertical-navigation-hover'                          : this._hovered,
            'gts-vertical-navigation-inner'                          : this.inner,
            'gts-vertical-navigation-mode-over'                      : this.mode === 'over',
            'gts-vertical-navigation-mode-side'                      : this.mode === 'side',
            'gts-vertical-navigation-opened'                         : this.opened,
            'gts-vertical-navigation-position-left'                  : this.position === 'left',
            'gts-vertical-navigation-position-right'                 : this.position === 'right',
        };
        /* eslint-enable @typescript-eslint/naming-convention */
    }
    ...
    /**
     * Setter for gtsScrollbarDirectives
     */
    @ViewChildren(GtsScrollbarDirective)
    set gtsScrollbarDirectives(gtsScrollbarDirectives: QueryList<GtsScrollbarDirective>)
    {
        // Store the directives
        this._gtsScrollbarDirectives = gtsScrollbarDirectives;

        // Return if there are no directives
        if ( gtsScrollbarDirectives.length === 0 )
        {
            return;
        }

        // Unsubscribe the previous subscriptions
        if ( this._gtsScrollbarDirectivesSubscription )
        {
            this._gtsScrollbarDirectivesSubscription.unsubscribe();
        }

        // Update the scrollbars on collapsable items' collapse/expand
        this._gtsScrollbarDirectivesSubscription =
            merge(
                ...
                .subscribe(() =>
                {
                    ...
                    gtsScrollbarDirectives.forEach((gtsScrollbarDirective) =>
                    {
                        gtsScrollbarDirective.update();
                    });
                });
    }
    ...
    ngOnInit(): void
    {
        ...
        if ( this.name === '' )
        {
            this.name = this._gtsUtilsService.randomId();
        }

        // Register the navigation component
        this._gtsNavigationService.registerComponent(this.name, this);
    ...
    ngAfterViewInit(): void
    {
        ...
        setTimeout(() =>
        {
            ...
            if ( !this._navigationContentEl.nativeElement.classList.contains('ps') )
            {
                ...
                const activeItem = this._navigationContentEl.nativeElement.querySelector('.gts-vertical-navigation-item-active');
            ...
            else
            {
                ...
                this._gtsScrollbarDirectives.forEach((gtsScrollbarDirective) =>
                {
                    // Skip if not enabled
                    if ( !gtsScrollbarDirective.isEnabled() )
                    {
                        return;
                    }

                    // Scroll to the active element
                    gtsScrollbarDirective.scrollToElement('.gts-vertical-navigation-item-active', -120, true);
                });
            }
        });
    }
    ...
    ngOnDestroy(): void
    {
        ...
        this._gtsNavigationService.deregisterComponent(this.name);
     ...
     * @param item
     */
    openAside(item: GtsNavigationItem): void
    {
     ...
     * @param item
     */
    toggleAside(item: GtsNavigationItem): void
    {
        // Toggle
    ...
    private _showOverlay(): void
    {
        ...
        this._overlay.classList.add('gts-vertical-navigation-overlay');

        // Add a class depending on the transparentOverlay option
        if ( this.transparentOverlay )
        {
            this._overlay.classList.add('gts-vertical-navigation-overlay-transparent');
        }
    ...
    private _showAsideOverlay(): void
    {
        ...
        this._asideOverlay.classList.add('gts-vertical-navigation-aside-overlay');
